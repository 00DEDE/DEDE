...
function fadeAmbient(targetVolume, durationMs) {
  ...
  // If we're fading UP from silence and the element is paused, resume it so
  // the volume ramp is actually audible.
  if (targetVolume > 0 && ambientAudio.paused && !ambientManuallyPaused) {
  ...
  function tick(now) {
    ...
    if (t < 1) {
      ...
      ambientFadeRAF = null;
      // Once the ramp lands at true zero, pause the element entirely so no
      // residual signal can leak under the overhead video's audio.
      if (targetVolume === 0) {
